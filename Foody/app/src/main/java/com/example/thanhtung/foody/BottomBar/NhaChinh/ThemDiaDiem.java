...
import com.example.thanhtung.foody.Login.Utility;
...
/**
 * Created by Thanh Tung on 5/21/2017.
 */
    ...
    public void getThemDiaDiem(String TenQuan, String DiaChi, String HinhAnh, String SDT, int MaDuong, int MaDanhMuc,
        ...
        try {
            if (MaDuongQuanThemDiaDiem != -1 && MaDanhMucThemDiaDiem != -1) {
                if (Utility.isNotNull(TenQuan) && Utility.isNotNull(DiaChi) && Utility.isNotNull(HinhAnh) && Utility.isNotNull(SDT)
                        && Utility.isNotNull(GioMoCua) && Utility.isNotNull(GioDongCua) && Utility.isNotNull(MoTa) && Utility.isNotNull(ThoiGianThem)) {
                    prgDialog.show();
                    ...
                    FoodyRestClient quanan = new FoodyRestClient();
                    quanan.themdiadiem(getApplicationContext(), "api/QuanAn/ThemQuanAn", entity, "application/json; charset=utf-8", new AsyncHttpResponseHandler() {
                        @Override
